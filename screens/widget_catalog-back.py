...
class WidgetCatalogScreen:
    ...
    def _build_weather(self, card, cfg):
        ...
        # Temperatur größer als der Rest (24pt -> 48pt, wie die Uhrzeit im
        # Uhr-Widget) - die "Hero-Zahl" dieser Kachel. Beschreibung/Stats
        # bleiben bei 24pt, sonst wird die Kachel zu unruhig.
        temp = card.add_label("--", font=lv.font_montserrat_48, color=COLORS["fg"])
    ...
    def _build_todo(self, card, cfg):
        ...
        for i, row in enumerate(rows):
            if i < len(items):
                ...
                row.set_text(to_ascii(prefix + it.get("text", "")))
                if it.get("done"):
    ...
    def refresh(self):
        ...
        for widget_id, parts in self._parts.items():
            kind = parts["kind"]
            if kind in ("clock", "todo", "pc_status"):
                # clock/todo: ticken selbst bzw. sind rein statisch.
    ...
    def update_pc_status(self):
        ...
        for widget_id in pc_status_ids:
            fetch_worker.submit(widget_id, _BACKGROUND_FETCHERS["pc_status"], self, self._parts[widget_id])
