...
class LocalBME688Source:
    ...
    def __init__(self, i2c=None, addr=BME688_I2C_ADDR, iaq_cfg=None,
        ...
        # Gespeicherte Baseline aus einem frueheren Lauf uebernehmen (hoechstens
        # 7 Tage alt; ist die Uhr noch nicht gestellt, wird sie akzeptiert).
        saved = iaq_cfg.get("saved_baseline") or 0
        if saved:
            age_s = time.time() - (iaq_cfg.get("saved_at") or 0)
            if age_s < 0 or age_s < 7 * 86400:
                if self.iaq_tracker.load_state(saved):
                    print("IAQ: gespeicherte Baseline %.0f Ohm uebernommen" % float(saved))
    ...
    def _read_raw(self):
        # Eine einzige Messung statt vier (siehe bme680_driver.read_all()).
        read_all = getattr(self._driver, "read_all", None)
...
class SensorManager:
    ...
    def __init__(self, remote_source, local_source, history=None,
        ...
        # Hauptthread blockieren (siehe fetch_worker.submit_cached()).
        self.remote_read_fn = None

    def _remote_read(self):
        if self.remote_read_fn is not None:
            return self.remote_read_fn()
    ...
    def read_all(self):
        ...
        return {"local": self.local.read(), "remote": self._remote_read()}
